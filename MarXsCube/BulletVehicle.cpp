...
void BulletVehicle::setup_wheels() {
	for (int i = 0; i < this->m_vehicle->getNumWheels(); i++) {
		...
		wheel.m_maxSuspensionForce = 100000.0f;
//		wheel.m_maxSuspensionTravelCm = 120*100.0;
	}
}

void BulletVehicle::launch() {
	for (int i = 0; i < this->m_vehicle->getNumWheels(); i++) {
		m_vehicle->applyEngineForce(100, i);
